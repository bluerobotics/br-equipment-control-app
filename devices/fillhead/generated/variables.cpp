...
int telemetry_build_message(const TelemetryData* data, char* buffer, size_t buffer_size) {
    ...
    // main_state
    if (pos < buffer_size) {
        pos += snprintf(buffer + pos, buffer_size - pos, "%s:%ld,", TELEM_KEY_MAIN_STATE, (long)data->main_state);
    }
    
    // injector_state
    if (pos < buffer_size) {
        pos += snprintf(buffer + pos, buffer_size - pos, "%s:%ld,", TELEM_KEY_INJECTOR_STATE, (long)data->injector_state);
    }
    
    // inj_valve_state
    if (pos < buffer_size) {
        pos += snprintf(buffer + pos, buffer_size - pos, "%s:%ld,", TELEM_KEY_INJ_VALVE_STATE, (long)data->inj_valve_state);
    }
    
    // vac_valve_state
    if (pos < buffer_size) {
        pos += snprintf(buffer + pos, buffer_size - pos, "%s:%ld,", TELEM_KEY_VAC_VALVE_STATE, (long)data->vac_valve_state);
    }
    
    // heater_state
    if (pos < buffer_size) {
        pos += snprintf(buffer + pos, buffer_size - pos, "%s:%ld,", TELEM_KEY_HEATER_STATE, (long)data->heater_state);
    }
    
    // vacuum_state
    if (pos < buffer_size) {
        pos += snprintf(buffer + pos, buffer_size - pos, "%s:%ld,", TELEM_KEY_VACUUM_STATE, (long)data->vacuum_state);
    }
    ...
    // injector_homed
    if (pos < buffer_size) {
        pos += snprintf(buffer + pos, buffer_size - pos, "%s:%ld,", TELEM_KEY_INJECTOR_HOMED, (long)data->injector_homed);
    }
