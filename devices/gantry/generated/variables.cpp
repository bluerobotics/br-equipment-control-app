...
int telemetry_build_message(const TelemetryData* data, char* buffer, size_t buffer_size) {
    ...
    // x_state
    if (pos < buffer_size) {
        pos += snprintf(buffer + pos, buffer_size - pos, "%s:%s,", TELEM_KEY_X_STATE, data->x_state);
    }
    
    // y_state
    if (pos < buffer_size) {
        pos += snprintf(buffer + pos, buffer_size - pos, "%s:%s,", TELEM_KEY_Y_STATE, data->y_state);
    }
    
    // z_state
    if (pos < buffer_size) {
        pos += snprintf(buffer + pos, buffer_size - pos, "%s:%s,", TELEM_KEY_Z_STATE, data->z_state);
    }
    ...
    // x_enabled
    if (pos < buffer_size) {
        pos += snprintf(buffer + pos, buffer_size - pos, "%s:%ld,", TELEM_KEY_X_ENABLED, (long)data->x_enabled);
    }
    
    // x_homed
    if (pos < buffer_size) {
        pos += snprintf(buffer + pos, buffer_size - pos, "%s:%ld,", TELEM_KEY_X_HOMED, (long)data->x_homed);
    }
    ...
    // y_enabled
    if (pos < buffer_size) {
        pos += snprintf(buffer + pos, buffer_size - pos, "%s:%ld,", TELEM_KEY_Y_ENABLED, (long)data->y_enabled);
    }
    
    // y_homed
    if (pos < buffer_size) {
        pos += snprintf(buffer + pos, buffer_size - pos, "%s:%ld,", TELEM_KEY_Y_HOMED, (long)data->y_homed);
    }
    ...
    // z_enabled
    if (pos < buffer_size) {
        pos += snprintf(buffer + pos, buffer_size - pos, "%s:%ld,", TELEM_KEY_Z_ENABLED, (long)data->z_enabled);
    }
    
    // z_homed
    if (pos < buffer_size) {
        pos += snprintf(buffer + pos, buffer_size - pos, "%s:%ld", TELEM_KEY_Z_HOMED, (long)data->z_homed);
    }
