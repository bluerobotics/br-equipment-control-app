/**
 * @file events.cpp
 * @brief Event sending implementation for the Gantry controller.
...
void sendEvent(Event event) {
    switch (event) {
        case EVENT_SCRIPT_RUN:
            {
                char buffer[256];
                snprintf(buffer, sizeof(buffer), "%s%s", EVENT_PREFIX, EVENT_STR_SCRIPT_RUN);
                sendMessage(buffer);
            }
            break;
        ...
        case EVENT_SCRIPT_RESET:
            {
                char buffer[256];
                snprintf(buffer, sizeof(buffer), "%s%s", EVENT_PREFIX, EVENT_STR_SCRIPT_RESET);
                sendMessage(buffer);
            }
            break;
...
void sendEventString(Event event, const char* param) {
    switch (event) {
        ...
        case EVENT_SCRIPT_RESET:
            {
                char buffer[256];
                snprintf(buffer, sizeof(buffer), "%s%s %s", EVENT_PREFIX, EVENT_STR_SCRIPT_RESET, param);
                sendMessage(buffer);
            }
            break;
