...
def generate_command_header(commands: Dict[str, Any], device_name: str) -> str:
    ...
    for category, cmds in categories.items():
        ...
        for cmd_name, cmd_data in cmds:
            ...
            help_text = cmd_data.get('description', cmd_data.get('help', 'No description available.'))
...
class CodeGeneratorDialog(tk.Toplevel):
    ...
    def export_to_firmware(self):
        ...
        try:
            ...
            exports = [
                (os.path.join(inc_dir, 'commands.h'), self.generated_commands),
                (os.path.join(src_dir, 'commands.cpp'), self.generated_commands_cpp),
                (os.path.join(inc_dir, 'variables.h'), self.generated_variables_h),
                (os.path.join(src_dir, 'variables.cpp'), self.generated_variables_cpp),
                (os.path.join(inc_dir, 'events.h'), self.generated_events_h),
                (os.path.join(src_dir, 'events.cpp'), self.generated_events_cpp),
            ]
            ...
            exported_paths = "\n".join(path for path, _ in exports)
            messagebox.showinfo(
                "Success",
                "Exported autogenerated files to firmware:\n\n"
                f"{exported_paths}"
            )
        except Exception as e:
            messagebox.showerror("Error", f"Failed to export files:\n{str(e)}")

    def _autosize_and_center(self, parent):
        """Resize window to fit content and center over parent."""
        ...
        margin = 80
        max_w = max(400, screen_w - margin)
        max_h = max(400, screen_h - margin)
        
        min_w = min(req_w, max_w)
        min_h = min(req_h, max_h)
        
        width = min(req_w + 40, max_w)
        height = min(req_h + 40, max_h)
